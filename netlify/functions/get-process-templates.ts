        ...
        if (templateIds.length > 0) {
            ...
            if (stepIds.length > 0) {
                const reqs = await sql`
                    SELECT ssr.step_id, ssr.supply_item_id, ssr.quantity_per, ssr.scales_with_output, ssr.scaling_mode,
                ...
                for (const req of reqs.rows) {
                    ...
                    supplyReqMap[req.step_id].push({
                        ...
                        scalingMode: req.scaling_mode || (req.scales_with_output === true ? 'per_output' : 'fixed'),
